...
class DeparturesCache {
        ...
        for (TransitLine line : schedule.getTransitLines().values()) {
            for (TransitRoute route : line.getRoutes().values()) {
                for (TransitRouteStop routeStop : route.getStops()) {
                    ...
                    double offset = routeStop.getDepartureOffset();
                    if (!Time.isUndefinedTime(offset)) {
                        ...
                        for (Departure dep : route.getDepartures().values()) {
                            departures.add(dep.getDepartureTime() + offset);
                        }
                    }
                }
            }
    ...
    double getNextDepartureTime(Id<TransitStopFacility> stopId, double depTime) {
