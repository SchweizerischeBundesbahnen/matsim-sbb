		...
		if (person != null) {
			...
			if (leg.getMode().equals(SBBModes.CAR)) {
				...
				if (actLinks.size() > 2) {
					...
					log.info("Mode of leg is car, but there are no proper links defined for person " + person.getId());
				}
			}
			if (actLinkIdsPerPerson.remove(person) == null) {
				throw new IllegalStateException("Person must be in vehicle " + person.getId());
			}
			if (actDistancePerPerson.remove(person) == null) {
				throw new IllegalStateException("Distance for person must be defined " + person.getId());
			}
		}
	}
}
