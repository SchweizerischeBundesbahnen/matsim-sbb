    ...
    public static void main(String[] args)    {
        ...
        for(TransitLine line: schedule_ref.getTransitLines().values())  {
            ...
            for(TransitRoute route: line.getRoutes().values())  {
                ...
                Assert.isTrue(route.getStops().size() == schedule_var.getTransitLines().get(line.getId()).getRoutes().get(route.getId()).getStops().size(), line.getId().toString() + " " + route.getId().toString());
                Assert.isTrue(route.getDepartures().size() == schedule_var.getTransitLines().get(line.getId()).getRoutes().get(route.getId()).getDepartures().size(), line.getId().toString() + " " + route.getId().toString());
                Assert.isTrue(route.getTransportMode().equals(schedule_var.getTransitLines().get(line.getId()).getRoutes().get(route.getId()).getTransportMode()), line.getId().toString() + " " + route.getId().toString());
                Assert.isTrue(route.getRoute().getLinkIds().size() == schedule_var.getTransitLines().get(line.getId()).getRoutes().get(route.getId()).getRoute().getLinkIds().size(), line.getId().toString() + " " + route.getId().toString());
            }
        }
        int routesVar = 0;
