 ...
 * <p>
 * Input: - TransitSchedule - TransitNetwork - CSV containing polylines for all Visum-Links - CSV containing ordered list of Visum-Links for each MATSim-Link
 * <p>
 * Output: - CSV file with polyline for each MATSim-Link, to be used as link-geometries in Via
 *
 * @author mrieser
 */
public class PolylinesCreator {
	...
	public void runPt(Network network, Visum visum, Map<Id<Link>, String> linkSequences, String outputPath) throws IOException {
		loadVisumPolylines(visum);
		loadLinkSequences(linkSequences);
		String matsimPolylinesFilePath = String.format("%s\\%s", outputPath, Filenames.POLYLINES);
	...
	private void processNetwork(String outputFilename) throws IOException {
		processNetwork(this.network, outputFilename);
	}
	...
	/**
	 * Calculates the square of the distance between the given coord and (x/y). This is a minor optimization, as we don't need the absolute distance, but just need to find the smallest one, and the
	 * smallest distance also has the smallest square distance, so we don't need to call the expensive Math.sqrt() method.
	 */
