...
/**
 * Calculates a zone-to-zone matrix containing service frequency for public transport.
 *
 ...
 *
 *
 *  Idea of the algorithm for a full matrix:
 ...
 * @author mrieser
 */
public class PTFrequencyMatrix {

    private PTFrequencyMatrix() {
    }

    public static <T> PTFrequencyMatrix.PtIndicators<T> calculateTravelTimeMatrix(SwissRailRaptorData raptorData, Map<T, SimpleFeature> zones, Map<T, Coord[]> coordsPerZone, double minDepartureTime, double maxDepartureTime, double stepSize, RaptorParameters parameters, int numberOfThreads) {
        // prepare calculation
        ...
        for (int i = 0; i < numberOfThreads; i++) {
            ...
            RowWorker<T> worker = new RowWorker<>(originZones, zones.keySet(), coordsPerZone, pti, raptor, parameters, minDepartureTime, maxDepartureTime, stepSize, counter);
    ...
    static class RowWorker<T> implements Runnable {
        ...
        RowWorker(ConcurrentLinkedQueue<T> originZones, Set<T> destinationZones, Map<T, Coord[]> coordsPerZone, PtIndicators<T> pti, SwissRailRaptor raptor, RaptorParameters parameters, double minDepartureTime, double maxDepartureTime, double stepSize, Counter counter) {
            ...
            while (true) {
                ...
                if (fromCoords != null) {
                    ...
                    for (T toZoneId : this.destinationZones) {
                        this.pti.adaptionTimeMatrix.set(fromZoneId, toZoneId, Float.POSITIVE_INFINITY);
                        this.pti.frequencyMatrix.set(fromZoneId, toZoneId, Float.POSITIVE_INFINITY);
        ...
        private void calcForOD(T fromZoneId, Coord fromCoord, T toZoneId, Coord toCoord, Map<Id<TransitStopFacility>, Double> accessTimes, List<Map<Id<TransitStopFacility>, TravelInfo>> trees) {
            ...
            this.pti.frequencyMatrix.set(fromZoneId, toZoneId, (float) ((maxDepartureTime - minDepartureTime) / avgAdaptionTime / 4.0));
        ...
        private List<ODConnection> buildODConnections(List<Map<Id<TransitStopFacility>, TravelInfo>> trees,  Map<Id<TransitStopFacility>, Double> egressTimes) {
            ...
            for (Map<Id<TransitStopFacility>, TravelInfo> tree : trees) {
                for (Map.Entry<Id<TransitStopFacility>, Double> egressEntry : egressTimes.entrySet()) {
                    ...
                    ODConnection connection = new ODConnection(info.ptDepartureTime, info.ptTravelTime, info.accessTime, egressTime, info.transferCount);
    ...
    static class ODConnection {
        ...
        ODConnection(double departureTime, double travelTime, double accessTime, double egressTime, double transferCount) {
