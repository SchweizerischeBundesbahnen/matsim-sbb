...
public class PTFrequencyMatrix {
    ...
    public static <T> PTFrequencyMatrix.PtIndicators<T> calculateTravelTimeMatrix(SwissRailRaptorData raptorData, Map<T, SimpleFeature> zones, Map<T, Coord[]> coordsPerZone, double minDepartureTime, double maxDepartureTime, double stepSize_seconds, RaptorParameters parameters, int numberOfThreads) {
        // prepare calculation
        ...
        for (int i = 0; i < numberOfThreads; i++) {
            ...
            RowWorker<T> worker = new RowWorker<>(originZones, zones.keySet(), coordsPerZone, pti, raptor, parameters, minDepartureTime, maxDepartureTime, stepSize_seconds, counter);
        ...
        private void calcForOD(T fromZoneId, Coord fromCoord, T toZoneId, Coord toCoord, Map<Id<TransitStopFacility>, Double> accessTimes, List<Map<Id<TransitStopFacility>, TravelInfo>> trees) {
            ...
            for (RaptorRoute.RoutePart part : route.getParts()) {
                if (part.line != null) {
                    ...
                    boolean isTrain = "SBB_Simba.CH_2016".equals(part.route.getAttributes().getAttribute("01_Datenherkunft"));
