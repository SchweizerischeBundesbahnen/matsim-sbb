        ...
        try (BufferedWriter out = IOUtils.getBufferedWriter(analysisFilename)) {
            ...
            for (Map.Entry<Id<TransitStopFacility>, List<LegData>> e1 : this.legData.entrySet()) {
                ...
                if (!accessDistances.isEmpty()) {
                    ...
                    out.write(stopId + "\t" + stop.getName() + "\t" + stop.getCoord().getX() + "\t" + stop.getCoord().getY() + "\t" + LegDataType.ACCESS + "\t" + minDist + "\t" + avgDist + "\t" + medDist + "\t" + maxDist + "\t" + accessDistances.size() + "\n");
                }
                ...
                if (!egressDistances.isEmpty()) {
                    ...
                    out.write(stopId + "\t" + stop.getName() + "\t" + stop.getCoord().getX() + "\t" + stop.getCoord().getY() + "\t" + LegDataType.EGRESS + "\t" + minDist + "\t" + avgDist + "\t" + medDist + "\t" + maxDist + "\t" + accessDistances.size() + "\n");
                }
            }
        } catch (IOException e) {
            log.error(e.getMessage(), e);
