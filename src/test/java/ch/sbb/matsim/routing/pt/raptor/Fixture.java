...
/*package*/ class Fixture {
	...
	protected void buildRedLine() {
		...
		{ // route from right to left
			...
			TransitRoute route = this.builder.createTransitRoute(Id.create("red G > C", TransitRoute.class), netRoute, stops, "rail");
			this.redLine.addRoute(route);
	...
	protected void buildGreenLine() {
		...
		{ // route in circle in clockwise
			...
			stops.add(stop);
			TransitRoute route = this.builder.createTransitRoute(Id.create("green clockwise", TransitRoute.class), netRoute, stops, "bus");
