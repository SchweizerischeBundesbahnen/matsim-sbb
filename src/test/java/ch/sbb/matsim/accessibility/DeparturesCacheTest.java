...
public class DeparturesCacheTest {
    ...
    public void testGetNextDepartureTime() {
        ...
        Assert.assertEquals(7*3600, cache.getNextDepartureTime(stopId, 6*3600), 1e-8);
        Assert.assertEquals(7*3600, cache.getNextDepartureTime(stopId, 7*3600), 1e-8);
        Assert.assertEquals(7*3600 + 20*60, cache.getNextDepartureTime(stopId, 7*3600 + 1), 1e-8);
        Assert.assertTrue(Time.isUndefinedTime(cache.getNextDepartureTime(stopId, 7*3600 + 40*60 + 1)));

        stopId = Id.create("2", TransitStopFacility.class);
        Assert.assertEquals(7*3600 + 300, cache.getNextDepartureTime(stopId, 7*3600), 1e-8); // line 1, route A
        Assert.assertEquals(7*3600 + 360, cache.getNextDepartureTime(stopId, 7*3600 + 301), 1e-8); // line 2, route B
        Assert.assertEquals(7*3600 + 10*60 + 420, cache.getNextDepartureTime(stopId, 7*3600 + 361), 1e-8); // line 2, route C
        Assert.assertEquals(7*3600 + 20*60 + 300, cache.getNextDepartureTime(stopId, 7*3600 + 1021), 1e-8); // line 1, route A
    }
    ...
    private static class Fixture {
        ...
        public Fixture() {
            ...
            stops.add(f.createTransitRouteStop(stop1, Time.getUndefinedTime(), 0));
            stops.add(f.createTransitRouteStop(stop2, 240, 300));
            stops.add(f.createTransitRouteStop(stop3, 600, Time.getUndefinedTime()));
            ...
            stops = new ArrayList<>();
            stops.add(f.createTransitRouteStop(stop4, Time.getUndefinedTime(), 0));
            stops.add(f.createTransitRouteStop(stop2, 300, 360));
            stops.add(f.createTransitRouteStop(stop5, 720, Time.getUndefinedTime()));
            ...
            stops.add(f.createTransitRouteStop(stop5, 780, Time.getUndefinedTime()));
